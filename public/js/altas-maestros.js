...
async function cargarReglasEan(){try{const d=await api('/api/altas-maestros/reglas-ean');const requiere=d.configuracion?.REQUIERE_EAN!==false;$('requiereEanEmpresa').value=String(requiere);$('requiereEanEmpresa').disabled=!puedeAdministrarEan;$('btnGuardarConfiguracionEanEmpresa').disabled=!puedeAdministrarEan;$('estadoGeneralEan').textContent=requiere?'Por defecto: requiere EAN':'Por defecto: no requiere EAN';$('estadoGeneralEan').className=`badge border ${requiere?'text-bg-success':'text-bg-secondary'}`;$('tablaReglasEan').innerHTML=(d.datos||[]).map(x=>`<tr><td>${textoSeguro(x.MARCA)}</td><td>${textoSeguro(x.RUBRO)}</td><td>${textoSeguro(x.LICENCIA||'Sin licencia')}</td><td><span class="badge ${x.REQUIERE_EAN?'text-bg-success':'text-bg-secondary'}">${x.REQUIERE_EAN?'REQUIERE EAN':'EAN NO REQUERIDO'}</span></td><td><div>${textoSeguro(x.USUARIO_ACTUALIZACION)}</div><small class="text-secondary">${textoSeguro(new Date(x.FECHA_ACTUALIZACION).toLocaleString('es-AR'))}</small></td><td>${puedeAdministrarEan?`<button class="btn btn-sm btn-outline-danger" data-eliminar-regla-ean="${x.ID_REGLA_EAN}">Eliminar</button>`:''}</td></tr>`).join('')||`<tr><td colspan="6" class="text-center text-secondary">No hay excepciones configuradas. Se aplica la configuración general: ${requiere?'requiere EAN':'no requiere EAN'}.</td></tr>`;}catch(e){alerta(e.message,'danger');}}
async function guardarConfiguracionEanEmpresa(){try{await api('/api/altas-maestros/reglas-ean/configuracion',{method:'PUT',body:JSON.stringify({requiereEan:$('requiereEanEmpresa').value==='true'})});alerta('Configuración general de EAN guardada correctamente.','success');await cargarReglasEan();}catch(e){alerta(e.message,'danger');}}
...
async function iniciar() {
  ...
  $('btnGuardarConfiguracionEanEmpresa').addEventListener('click',guardarConfiguracionEanEmpresa);

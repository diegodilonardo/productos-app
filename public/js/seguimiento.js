...
function pintarTarjetasSeguimiento(filas) {
  ...
  contenedor.innerHTML = filas.map(fila => {
    ...
    return `<article class="seguimiento-summary-card seguimiento-summary-${estado.toLowerCase().replaceAll('_', '-')}">
      ...
      ${Number(fila.CANTIDAD_PRODUCTOS_INHABILITADOS || 0) > 0 ? `<div class="my-2"><span class="badge text-bg-danger">${Number(fila.CANTIDAD_PRODUCTOS_INHABILITADOS)} producto${Number(fila.CANTIDAD_PRODUCTOS_INHABILITADOS) === 1 ? '' : 's'} inhabilitado${Number(fila.CANTIDAD_PRODUCTOS_INHABILITADOS) === 1 ? '' : 's'} en Presea</span></div>` : ''}

...
function pintarEdicionUsuario(usuario) {
  ...
  const accesosHtml = usuario.superAdmin
    ...
    `
    : accesos.map(acceso => crearTarjetaAcceso(acceso)).join('');
...
function crearTarjetaAcceso(acceso = {}) {
  ...
  return `
    <div class="border rounded-3 p-3 acceso-edicion" data-indice-acceso="${indice}">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <button class="btn btn-outline-danger btn-sm btn-quitar-acceso" type="button">Quitar</button>
      </div>

      <div class="row g-3 mb-3">
        <div class="col-md-7">
          <label class="form-label">Empresa</label>
          <select class="form-select campo-empresa" required>
        ...
        <div class="col-md-5">
          ...
          <select class="form-select campo-rol" required>
            <option value="">Seleccionar...</option>
            ${rolesOptions}
          </select>
        </div>
...
async function guardarUsuario(event) {
  ...
  try {
    const payload = {
      activo: Boolean(document.getElementById('usuarioActivo')?.checked),

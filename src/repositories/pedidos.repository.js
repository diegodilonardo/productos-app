  ...
  const resultado = await pool.request()
    ...
    .query(`
      SELECT
        ...
        (
          SELECT COUNT(*) FROM dbo.PEDIDOS_DETALLE DI
          INNER JOIN dbo.PRODUCTOS PI
            ON PI.ID_EMPRESA = DI.ID_EMPRESA AND PI.ID_PRODUCTO = DI.ID_PRODUCTO
          WHERE DI.ID_EMPRESA = P.ID_EMPRESA AND DI.ID_PEDIDO = P.ID_PEDIDO
  ...
  const resultado = await pool.request()
    ...
    .query(`
      SELECT PD.*, A.CODIGO_ANO, A.CODIGO_TEMPORADA, A.CODIGO_ALTA,
        ORIGEN.SEXO, PR.C_ESTADIO,
        CASE WHEN LTRIM(RTRIM(ISNULL(PR.C_ESTADIO, ''))) = '9'
